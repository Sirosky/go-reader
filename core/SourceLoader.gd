...
func source_load(dir): #Loads and sorts all the source images
	print(ProjectSettings.globalize_path(dir))
	Main.cur = 0 #Reset whatever we were reading
	var search
	#OS.shell_open("C:\\Users")
	#OS.shell_open(ProjectSettings.globalize_path("user://"))
	
	#1- Search loaded directory for importing if necessary
	if dir != "user://":
	...
	#2- Search new directory
	search = file_search.search_regex_full_path(filter_regex, "user://temp/", 1)
