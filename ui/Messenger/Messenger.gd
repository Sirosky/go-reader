...
var message_life = 120 #How long they should stay
var message_timer_arr: Array #Holds timer info for all messages
var message_alpha = .8 #Maximum alpha value
var message_offset_y = 46 #How much to offset landing spot
...
func message_sort():
	...
	for i in message_queue:
		...
		if index != 0:
			...
			var tar_y = window_height - ((i.rect_size.y + message_gap) * (index + 1)) - message_offset_y
			
			global.Tween.interpolate_property(i, "rect_position", Vector2(i.rect_position.x, i.rect_position.y), Vector2(tar_x,tar_y), 1.5, global.Tween.TRANS_SINE, global.Tween.EASE_OUT)
			global.Tween.start()
...
func message_delete(message):
	...
	global.Tween.interpolate_property(message, "modulate",Color(1, 1, 1, message_alpha), Color(1, 1, 1, 0), tween_dur, global.Tween.TRANS_SINE, global.Tween.EASE_OUT)
	global.Tween.start()
